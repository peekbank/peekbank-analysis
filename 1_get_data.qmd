...
```{r}
load(file = here("..", "peekbank-method", "cached_intermediates", "1_d_trial.Rds"))
d_aoi <- d_trial # horrible old naming convention, FIXME
```
...
Compute RTs, relying on the RLE workflow from the shiny app. (we tested this back in the day at least). 
...
```{r}
d_trial_rt <- d_trial_rt_unfiltered |>
...
```{r}
saveRDS(d_trial_rt, here("cached_intermediates","1_d_trial_rt"))
```
