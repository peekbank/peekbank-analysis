...
load(file = here("data","1_d_trial.Rds"))
```
...
preds <- expand_grid(
  age = seq(min(d_acc$age), max(d_acc$age),1),
...
```{r}
mod_acc <- lmer(elogit ~ log_age_centered * animate_target + 
...
```{r}
save(file = here("data","3A_d_acc.Rds"), d_acc)
save(file = here("data","3A_mod_acc.Rds"), mod_acc)
```
...
preds <- preds |>
  mutate(.pred = predict(lmm_fit_bc, 
                         ...
                         re.form = ~  (log_age_centered | target_label), 
                         newdata = preds), 
         age = exp(log_age_centered + mean(d_acc_bc$log_age)))
...
mod_acc_bc <- lmer(elogit_baseline ~ log_age_centered * animate_target + 
                  log_age_centered * animate_distractor +
                  (1 | administration_id) + 
                  (log_age_centered | dataset_name) + 
                  (log_age_centered | target_label) + 
...
preds <- preds |>
  mutate(.pred = predict(mod_acc_bc, 
                         type = "response",
                         re.form = ~  (log_age_centered | target_label), 
                         newdata = preds), 
         age = exp(log_age_centered + mean(d_acc_bc$log_age)))
...
```{r}
save(file = here("data","3A_d_acc_bc.Rds"), d_acc_bc)
save(file = here("data","3A_lmm_fit_bc.Rds"), mod_acc_bc)
```
