...
```{r}
sa <- d_summary |> 
  filter(dataset_name == "reflook_v4")
...
icc_window_sim <- function (t_start = -500, t_end = 4000, object) 
{
  ...
  df <- d_trial |>
    ...
    #make sure to filter NAs for accuracy
    filter(!is.na(accuracy))
...
acc_params_extended <- expand_grid(t_start = seq(-1000,1500,50),
                          t_end = seq(2000,4000,50),
                          object = c("stimulus", "administration")) |>
  mutate(icc = pmap(list(t_start, t_end, object), icc_window_sim)) |>
  unnest(col = icc)

save(file = "cached_intermediates/2_acc_params.Rds", acc_params)
save(file = "cached_intermediates/2_acc_params_extended.Rds", acc_params_extended)
```
...
ggplot(filter(acc_params_summary,object=="administration"),aes(color=mean_icc))+
  ...
  annotate("text", x = -600, y = 0.45, label = "Target Onset", size=7) +
  ...
  scale_y_continuous(breaks=seq(0.15,0.45,0.1),limits=c(0.15,0.45))+
...
```{r}
load(file = "cached_intermediates/2_acc_params_extended.Rds")

acc_params_extended_summary <- acc_params_extended %>%
  group_by(t_start,t_end,object) %>%
  summarize(
    N=n(),
    mean_icc=mean(icc,na.rm=TRUE)
...
ggplot(filter(acc_params_extended_summary,object=="administration"),aes(x=t_start,y=t_end,fill=mean_icc))+
  geom_tile(color="white")+
  scale_fill_viridis(name="Mean ICC\n(by-subject)",option="inferno")+
  theme_cowplot()+
  scale_x_continuous(breaks=c(-1000,-500,0,500,1000,1500))+
  xlab("Window Start Time (in ms)")+
  ylab("Window End Time (in ms)")
ggsave(here(figure_path,"reliability_window_grid_ICC_subject_extended.png"),width=10,height=6,dpi=600)

ggplot(acc_params_extended_summary,aes(x=t_start,y=t_end,fill=mean_icc))+
  geom_tile(color="white")+
  scale_fill_viridis(name="Mean ICC",option="inferno")+
  theme_cowplot()+
  scale_x_continuous(breaks=c(-1000,-500,0,500,1000,1500))+
  ...
  ylab("Window End Time (in ms)")+
  facet_wrap(~object)
ggsave(here(figure_path,"reliability_window_grid_overall_extended.png"),width=10,height=6,dpi=600)
...
#-1000,1500,50
acc_params_byage <- expand_grid(t_start = seq(-1000,1500,50),
                                t_end = seq(2000,4000,50),
                                object = c("stimulus", "administration")) |>
  mutate(icc = pmap(list(t_start, t_end, object), icc_window_sim)) |>
  unnest(col = icc)
...
icc_window_sim <- function (t_start = 0, t_end = 4000, object) 
{
  df <- d_trial |>
    ...
    filter(t_norm > t_start, t_norm < t_end) |>
    group_by(dataset_name, dataset_id, age_bin, administration_id, 
             target_label, trial_id) |>
    summarise(accuracy = mean(correct, na.rm=TRUE),
              prop_data = mean(!is.na(correct)))
...
acc_params_byagebins <- expand_grid(t_start = seq(0,1500,500),
                                t_end = seq(2000,4000,500),
                                object = c("stimulus", "administration")) |>
  mutate(icc = pmap(list(t_start, t_end, object), icc_window_sim)) |>
  unnest(col = icc)

save(file = "cached_intermediates/2_acc_params_byagebins.Rds", acc_params_byage)
```

Now plot. 

```{r}
load(file = "cached_intermediates/2_acc_params_byagebins.Rds")

ggplot(acc_params_byagebins, aes(x = t_start, y = icc, col = as.factor(t_end))) + 
  geom_jitter() + 
  facet_grid(age_bin~object) + 
  geom_smooth(aes(group = as.factor(t_end)), se = FALSE)
```


```{r}
acc_params_byagebins_summary <- acc_params_byagebins %>%
  group_by(age_bin,t_start,t_end,object) %>%
  summarize(
    N=n(),
    mean_icc=mean(icc,na.rm=TRUE)
  ) %>%
...
ggplot(filter(acc_params_byagebins_summary,object=="administration"),aes(x=t_start,y=t_end,fill=mean_icc))+
  geom_tile(color="white")+
  scale_fill_viridis(name="Mean ICC\n(by-subject)",option="inferno")+
  theme_cowplot()+
  scale_x_continuous(breaks=c(-1000,-500,0,500,1000,1500))+
  ...
  ylab("Window End Time (in ms)")+
  facet_wrap(~age_bin)
ggsave(here(figure_path,"reliability_window_grid_ICC_subject_byagebins.png"),width=10,height=6,dpi=600)
```
